...
def migrar_catalogo(sqlite_precios: Path, sqlite_apus: Path, cx: Conexion,
    ...
    n = {"insumos": 0, "precios": 0, "apus": 0, "componentes": 0, "listas": 0}
    try:
        with cx.connection() as conn:
            ...
            # listas de precios (id explícito para preservar el linkage con
            # insumo_precios.lista_id) — VA ANTES de insumos/insumo_precios: el
            # orden importa por la FK insumo_precios.lista_id -> lista_precios.id.
            # 'Principal' (id 1) ya existe en destino (la siembra precios.sql en
            # el boot); ON CONFLICT DO NOTHING la deja intacta y solo agrega las
            # demás (p.ej. listas NP).
            # lista_precios.id es GENERATED BY DEFAULT (no ALWAYS) -- igual que el
            # INSERT de 'Principal' en db/pg/precios.sql, el id explícito se acepta
            # sin OVERRIDING SYSTEM VALUE (esa cláusula es solo para columnas ALWAYS,
            # como precios.insumos.id / insumo_precios.id de abajo).
            filas = sp.execute("SELECT id, nombre, creada_en, creado_por "
                               "FROM lista_precios").fetchall()
            lst = [(r["id"], r["nombre"], r["creada_en"], r["creado_por"]) for r in filas]
            with conn.cursor() as cur:
                cur.executemany(
                    "INSERT INTO precios.lista_precios (id, nombre, creada_en, creado_por) "
            ...
            # lista_id viaja con cada fila: sin esto, TODAS las filas migradas
            # caerían en el DEFAULT 1 (Principal) y una lista NP con precio
            # vigente propio quedaría como una 2ª fila vigente=1/lista_id=1 —
            # cruce ambiguo en get_candidatos (ver Hallazgo 3 del review).
            filas = sp.execute("SELECT id, insumo_id, precio, fuente, clasificacion, fecha, "
                               "vigente, lista_id FROM insumo_precios").fetchall()
            prc = [(r["id"], r["insumo_id"], r["precio"], r["fuente"],
                    r["clasificacion"], r["fecha"], r["vigente"], r["lista_id"]) for r in filas]
            with conn.cursor() as cur:
                cur.executemany(
                    "INSERT INTO precios.insumo_precios "
                    "(id, insumo_id, precio, fuente, clasificacion, fecha, vigente, creado_por, "
                    "lista_id) "
                    "OVERRIDING SYSTEM VALUE VALUES (%s,%s,%s,%s,%s,%s,%s,'migración',%s) "
                    "ON CONFLICT DO NOTHING", prc)
            n["precios"] = len(prc)
            _resync_identity(conn, "precios.lista_precios")
...
def verificar(sqlite_precios: Path, sqlite_apus: Path, cx: Conexion) -> dict:
    ...
    try:
        origen = {
            "lista_precios": sp.execute("SELECT COUNT(*) FROM lista_precios").fetchone()[0],
    ...
    with cx.connection() as conn:
        destino = {
            "lista_precios": conn.execute(
                "SELECT COUNT(*) AS n FROM precios.lista_precios").fetchone()["n"],
