...
def mul_redondeado(a: float, b: float) -> int:
    ...
    - ``a * b <= 0`` (incluido NaN) -> 0 (0 genuino; lo marca la alerta de costeo).
    - ``a * b > 0`` que redondearía a 0 -> 1 (nada en $0 por redondeo).
    - resto -> ``floor(a * b + 0.5)``.
    """
    p = a * b
    if not (p > 0):
        # `not (p > 0)` y NO `p <= 0`: con NaN, `nan <= 0` es False (las comparaciones
        # con NaN son siempre False) y se colaría a `math.floor(nan + 0.5)`, que
        # revienta con ValueError. Mismo patrón ya usado en corridas.py/seqs_sin_apu.
        return 0
    r = math.floor(p + 0.5)
