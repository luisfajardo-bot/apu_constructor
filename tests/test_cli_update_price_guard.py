...
def test_update_price_rechaza_negativo(capsys, monkeypatch):
    # Un precio negativo debe rechazarse igual que 0: sin escritura, código de error.
    llamado = {"set": False}
    ...
    monkeypatch.setattr(cli, "get_almacen", lambda: _Alm())
    rc = cli.cmd_db_update_price(SimpleNamespace(codigo="7", precio=-5.0, nombre=None, fuente=None))
    assert rc == 1
    assert llamado["set"] is False
    assert "mayor que 0" in capsys.readouterr().out
